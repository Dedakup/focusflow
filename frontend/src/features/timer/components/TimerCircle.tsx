...
const TimerCircle: React.FC<TimerCircleProps> = ({ radius }) => {
    const dispatch = useAppDispatch();
    const timeLeft = useAppSelector((state) => state.pomodoro.timeLeft);
    const isActive = useAppSelector((state) => state.pomodoro.isRunning);
    const phase = useAppSelector((state) => state.pomodoro.currentPhase);
    const hasStarted = useAppSelector((state) => state.pomodoro.hasStarted);
    ...
    return (
        ...
        >
            ...
            <div className="absolute flex flex-col items-center">
                <div
                    className={`mb-4 transition-all duration-500 ${isActive ? 'transform -translate-y-[40px]' : ''}`}
                >
                    {phase === 'focus' && isActive ? (
                        ...
                        >
                            <g clipPath="url(#clip0_24_4693)">
                                <path
                                    d="M21 9.33317H22.1667C23.4043 9.33317 24.5913 9.82484 25.4665 10.7C26.3417 11.5752 26.8333 12.7622 26.8333 13.9998C26.8333 15.2375 26.3417 16.4245 25.4665 17.2997C24.5913 18.1748 23.4043 18.6665 22.1667 18.6665H21M21 9.33317H2.33333V19.8332C2.33333 21.0708 2.82499 22.2578 3.70016 23.133C4.57533 24.0082 5.76232 24.4998 6.99999 24.4998H16.3333C17.571 24.4998 18.758 24.0082 19.6332 23.133C20.5083 22.2578 21 21.0708 21 19.8332V9.33317ZM6.99999 1.1665V4.6665M11.6667 1.1665V4.6665M16.3333 1.1665V4.6665"
                                    stroke="#F3F3F3"
                                    strokeWidth="2.5"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                ...
                <span
                    className={`text-white transition-all duration-500 ${isActive ? 'text-6xl' : 'text-4xl'}`}
                >
                    {formatTime(timeLeft)}
                </span>
                {hasStarted && !isActive && (
